# VibeChat — Sprint Implementation Checklist
...
**Team:** 2 engineers + 1 QA/DevOps (part-time)
...
- [ ] `npm run dev` connects to all three databases without errors
- [ ] `prisma migrate dev --preview-feature` runs successfully
...
**✅ Acceptance Criteria:**
- [ ] User can sign up with valid email and receives verification email placeholder
...
- [ ] Implement Redis key patterns: `presence:{uid}`, `session:{uid}`, `socket:{socket_id}`, `unread:{uid}:{cid}`, `typing:{cid}`, `seq:{cid}`, `msg:lock:{cid}`
...
- [x] Implement Lua scripts (partial): lock release (ownership check) implemented via Lua
- [ ] Test: cold start sequence seeding, lock contention under concurrent writes, Lua script atomicity
...
- [x] Check membership in PostgreSQL before responding; cache recent membership in Redis (TTL 1h)
...
- [x] Redis tests: sequence counter increments atomically; typing indicators expire and presence TTL implemented
- [x] Concurrency tests: two servers INCR same channel's seq counter in parallel
...
### Performance Targets
- **Send Message:** <200ms end-to-end
- **Fetch History:** <500ms for 50 messages
- **Realtime Broadcast:** <500ms from send to on-screen
- **Concurrency:** 100 msg/sec sustained throughput
...
**Note:** Sprint lengths assume a small team (2 engineers + 1 QA/DevOps part-time). Adjust story points accordingly. Break large items into smaller tickets before sprint planning. Keep at most 3 high-risk items per sprint to limit scope creep.